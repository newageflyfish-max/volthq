 ...
 */
import { DEFAULT_ROUTING_PROFILE, } from './types.js';
const WEIGHTS = {
 ...
 */
export function scoreOfferings(offerings, profile = DEFAULT_ROUTING_PROFILE) {
    ...
    const scored = eligible.map(offering => {
        ...
        const score = weights.quality * qualityComponent +
 ...
 */
export function generateRecommendation(offerings, profile = DEFAULT_ROUTING_PROFILE, currentCostPerMillion = null) {
    const scored = scoreOfferings(offerings, profile);
    if (scored.length === 0)
        return null;
    const recommended = scored[0];
    const alternatives = scored.slice(1, 4); // top 3 alternatives
    const recommendedCost = (recommended.offering.priceInputPerMillion +
        recommended.offering.priceOutputPerMillion) / 2;
 ...
 */
export function comparePrices(offerings, modelQuery) {
    const matching = offerings.filter(o => {
        const query = modelQuery.toLowerCase();
        return (o.model.toLowerCase().includes(query) ||
