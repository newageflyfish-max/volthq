 ...
 */
export function makeOfferingId(
  providerId: string,
  model: string,
 ...
 */
const TOKENS_PER_GPU_HOUR: Record<string, Record<string, number>> = {
 ...
 */
export function gpuHourToPerToken(
  pricePerHour: number,
  modelName: string,
  gpuType: string
 ...
 */
export function assignCapabilityTier(modelName: string): 1 | 2 | 3 | 4 | 5 {
  const m = modelName.toLowerCase();
  
  // Tier 1: Frontier
  if (m.includes('gpt-4o') && !m.includes('mini')) return 1;
  if (m.includes('claude-3-opus') || m.includes('claude-opus')) return 1;
  if (m.includes('405b')) return 1;
  if (m.includes('deepseek-v3')) return 1;
  if (m.includes('deepseek-r1') && !m.includes('distill')) return 1;
  
  // Tier 2: Strong
  if (m.includes('gpt-4o-mini')) return 2;
  if (m.includes('claude-3-sonnet') || m.includes('claude-sonnet')) return 2;
  if (m.includes('70b')) return 2;
  if (m.includes('qwen2.5-72b')) return 2;
  if (m.includes('mixtral')) return 2;
  
  // Tier 3: Good
  if (m.includes('8b') || m.includes('7b')) return 3;
  if (m.includes('gemma')) return 3;
  if (m.includes('phi-')) return 3;
  
  // Tier 4: Fast/cheap
  if (m.includes('flash') || m.includes('haiku') || m.includes('mini')) return 4;
  if (m.includes('3b') || m.includes('1b')) return 4;
  
  // Tier 5: Specialized
  if (m.includes('embed') || m.includes('whisper')) return 5;
