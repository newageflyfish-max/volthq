 ...
 */
import { z } from 'zod';
import { generateRecommendation, } from '@volthq/core';
...
export function handleRecommendRoute(input, feedCache) {
    ...
    const modelOfferings = allOfferings.filter((o) => o.model.toLowerCase().includes(query) || o.modelShort.toLowerCase().includes(query));
    ...
    const rec = generateRecommendation(modelOfferings, profile, input.current_cost_per_million);
    if (!rec) {
        return {
            content: [
                {
    ...
    return {
        content: [
            {
                type: 'text',
                text: formatRecommendation(rec, input.optimize),
            },
        ],
    };
}
function formatRecommendation(rec, optimize) {
    ...
    if (rec.alternatives.length > 0) {
        ...
        for (const alt of rec.alternatives) {
            ...
            lines.push(`  - ${ao.providerName} ${ao.modelShort}${aq}: $${altAvg.toFixed(2)}/M avg, score ${(alt.score * 100).toFixed(1)}`);
        }
    }
    return lines.join('\n');
}
//# sourceMappingURL=recommend-route.js.map